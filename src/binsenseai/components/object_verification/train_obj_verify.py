...
#  to display a pair of images side by side, along with their corresponding target labels.
...
#or saving concatenated image pairs to files for later inspection or analysis during validation. 
# Each JPEG file will contain multiple image pairs concatenated vertically.
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        data_utils_obj_verify.ImageFolderSiamesePair(args.data, './artifacts/data_transformation/obj_verification_train.json', 0.5, transforms.Compose([
    ...
    val_loader = torch.utils.data.DataLoader(
        data_utils_obj_verify.ImageFolderSiamesePairVal(args.data, './artifacts/data_transformation/obj_verification_val.json', args.max_target, transforms.Compose([
            transforms.ToTensor(),
            normalize,
        ])),
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (im1, im2, target) in enumerate(train_loader):
        ...
        # Compute loss
        loss = criterion(output, target.view(-1, 1))  # Reshape target to match output size
        ...
        # Compute accuracy
        accuracy = (output > 0.5).float().eq(target).float().mean()

        # Record statistics
        losses.update(loss.item(), im1.size(0))
        train_acc.update(accuracy.item(), im1.size(0))
